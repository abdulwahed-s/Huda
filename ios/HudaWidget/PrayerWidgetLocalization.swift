...
enum PrayerTimeFormatter {
    ...
    static func formatDevice(
        ...
        languageCode: String,
        format: PrayerWidgetTimeFormat = .system,
        timeZone: TimeZone = .current
    ) -> String {
        let formatter = DateFormatter()
        formatter.timeZone = timeZone
        formatter.locale = Locale(identifier: languageCode)
        let uses24HourClock: Bool = switch format {
        case .system: deviceUses24HourClock
